...
def solve_milp_crowdshipping(instance_data, timelimit=None, nodelimit=None):
    """
    MILP with integrated cuts
    """
    try:
        model = gp.Model("PTCP_Crowdshipping_Final")
        ...
        #################### VALID INEQUALITIES
        #(24)if the source station i is selected at least one parcel must start its path at it
        model.addConstrs((gp.quicksum(s_ip[i,p] for p in P) >= e_i[i] for i in S), name="VI1")
        #(25)destination station can't be selected for parcel p if the parcel has not been assigned to a crowdshipper
        model.addConstrs((gp.quicksum(d_ip[i,p] for i in D_p[p] if i in D) <= gp.quicksum(y_kp[k,p] for k in K) for p in P), name="VI2")
        #(26)a source station can't be selected for parcel p if the parcel has not been assigned to a crowdshipper
        model.addConstrs((gp.quicksum(s_ip[i,p] for i in S) <= gp.quicksum(y_kp[k,p] for k in K) for p in P), name="VI3")
        #(27) parcel p can leave a station only if a crowdshipper handles this parcel
        model.addConstrs((gp.quicksum(l_ipt[i,p,t] for i in N for t in T) <= gp.quicksum(y_kp[k,p] for k in K) for p in P), name="VI4")
        #(28) parcel p can arrive at a station only if a crowdshipper handles this parcel
        model.addConstrs((gp.quicksum(a_ipt[i,p,t] for i in N for t in T) <= gp.quicksum(y_kp[k,p] for k in K) for p in P), name="VI5")
        #(29) a parcel cannot be assigned to a crowdshipper if it has already been allocated to the backup service 
        model.addConstrs((y_kp[k,p] <= 1 - q_p[p] for k in K for p in P), name="VI6")
        #(30) for a parcel p source and destination variables cannot be both equal to 1
        model.addConstrs((d_ip[i,p] + s_ip[i,p] <= 1 for i in N for p in P), name="VI7")
        #(31)-(32) the location variables for parcel p at time slot t can only be active at a station if both a starting and a destination stations have been assigned to this parcel
        model.addConstrs((gp.quicksum(o_ipt[i, p, t] for i in N) <= gp.quicksum(s_ip[i, p] for i in S) for p in P for t in T), name="VI8")
        model.addConstrs((gp.quicksum(o_ipt[i, p, t] for i in N) <= gp.quicksum(d_ip[i, p] for i in D) for p in P for t in T), name="VI9")
...
# instance_filepath = os.path.join("instances", "final_instance_dopo_natale_prova.json")

# with open(instance_filepath, "r") as f:
#     instance_data = json.load(f)  

# results = \
#     solve_milp_crowdshipping(instance_data, timelimit=1000)
# if results:

    ...
    def _generate_pt_network_from_curves(
        ...
        for station_id_counter, coords in enumerate(sorted_coords):
            ...
            station = Station(
                ...
                capacity_apl=capacity_apl, loading_cost=round(rng.uniform(0.5, 2), 3),
...
def generate_realistic_txt_dataset(
    ...
    # Identifica gli Hub
    hubs = [s.id for s in apl_stations if s.is_exchange_station]
    others = [s.id for s in apl_stations if not s.is_exchange_station]

    weights = []
    for s_id in station_ids:
        if s_id in hubs:
            weights.append(10) # high weight Hub
        else:
            weights.append(1)  # low weight for the others

    for _ in range(num_cs):
        # (Weighted Random Sampling)
        origin = rng.choices(station_ids, weights=weights, k=1)[0]
        
        dest_candidates = [s for s in station_ids if s != origin]
        dest_weights = [weights[i] for i in dest_candidates] # riallinea i pesi
        dest = rng.choices(dest_candidates, weights=dest_weights, k=1)[0]
        
        # departures in peak hours (es. slot 0-10)
        departure_slot = rng.randint(0, 10)
...
def make_instance_harder(generator_output_lines):
    new_lines = []
    iterator = iter(generator_output_lines.split('\n'))
    
    current_section = ""
    
    for line in iterator:
        # 1. FIX travel times (GRAPH)
        if line.startswith("GRAPH"):
            parts = line.split()
            new_lines.append(line) # Header GRAPH N
            num_nodes = next(iterator) # N nodes
            new_lines.append(num_nodes)
            
            while True:
                arc_line = next(iterator)
                if arc_line.startswith("END"):
                    new_lines.append("END")
                    break
                
                u, v, t = arc_line.split()
                # forces time [5, 15] with symmetry u-v and v-u
                new_t = random.randint(5, 15) 
                new_lines.append(f"{u} {v} {new_t}")
            continue

        # 2. FIX COSTI BACKUP 
        if line.startswith("BACKUP-COST"):
            new_lines.append(line)
            while True:
                cost_line = next(iterator)
                if cost_line.startswith("END"):
                    new_lines.append("END")
                    break
                
                val = float(cost_line)
                new_val = max(12, min(20, val)) # Clamp to paper range
                new_lines.append(f"{int(new_val)}")
            continue
...
if __name__ == '__main__':
    ...
    def hardy_travel_time(id1, id2, speed=1.0):
        return random.randint(5, 15) 
    
    my_generator._calculate_travel_minutes_between_stations = hardy_travel_time
    ...
    dataset_txt = generate_realistic_txt_dataset(
        ...
        num_cs=75,
        num_parcels=50,
        backup_cost_range=(5, 8),
        step_size=5,  
        step_count=85   
    )
    # 3. Save
    os.makedirs("instances", exist_ok=True)
    txt_path = "instances/instance_harder.txt"
    ...
    json_path = "instances/instance_harder.json"
